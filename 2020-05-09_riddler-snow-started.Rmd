...
I am basically going to simulate the process with very small time steps and track the location of each person.
These points can then be used to measure the distance traveled by each person.
...
Below are a few constants to define from the beginning.
`time_step_size` is the distance traveled at each time step of the simulation.
`minimum_distance` is the minimum safe distance that the two people need to stay from each other.
...
The `update_tracker()` function updates a data table with a person's current location for a given time step.
...
The `move_person()` function moves a person left or right by a given amount `x`.
...
The `shift_person()` function shifts a person up or down by a given about `y`.
...
The `measure_distance()` function measures the distances between two $(x, y)$ coordinates `a` and `b`.
...
The following for-loop moves each person, `A` and `B`, towards each other from 12 feet away.
At each step, the distance between one another is measured, and if they are too close to each other, they are slowly moved apart until they are again safe.
The movement of each person is tracked in a data table.

One complication I ran into was how to move the people back towards the middle after they passed each other.
Instead, I realized that the problem is symmetric, so I could just run the first half of the simulation - from 12 to 6 feet apart - and then use the symmetry to get the second half.
...
The plot below shows the path of `A` for the first half of the simulation.
...
The simulation was completed by copying the tracker data tables for `A` and `B` and combing these into a single `people_tracker` data table.
...
Finally, I could plot the paths taken by the two individuals while always remaining as close to the middle of the sidewalk as possible and keeping 6 feet apart.
...
The social distancing rules guides are shown in the plot below, they they overlap a lot because there is no time dimension.
...
I used ['gganimate'](https://gganimate.com) to show the two people walking towards each other with their 6-foot social distancing guides.
...
Finally, as the Riddler requested, I calculated the total distance traveled by person `A`.
...
**Each person travels a total of `r total_distance` ft, `r total_distance - 12` more feet than without the need for social distancing.**
